...
def sudoku_solve(sudoku):
    ...
    if row is None: #this is true if the find_empty_box function returns None, None
        return True
    ...
    for guess in range(1, 10): #guess can be any number from 1 to 9
        ...
        sudoku[row][col] = -1 #if not valid or if nothing gets returned true, then it needs to backtrack and try a new number
    
    return False #if none of the numbers work, then this puzzle is UNSOLVABLE!!
